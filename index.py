...
def command():
    ...
    try:
        ...
        e.delete(0, END)
        e.insert(0, f"You said : {ask}")
        engine.say(f"You said : {ask}")
        engine.runAndWait()
    except Exception:
        e.delete(0, END)
        e.insert("Say that again please...")
...
def GSearch():
    search_query = e.get()
    e.delete(0, END)
    for i in search(search_query, tld="co.in", lang="en", num=1, stop=1, pause=2.0):
        linkk = i
        print(i)
        e.insert(0, i)
...
def reminder():
    ...
    engine.say("Your reminder has been triggered")
    engine.runAndWait()
...
intropew = customtkinter.CTkLabel(
    root,
    text="****************************************************************************************",
)
intropew.pack()
...
button_6 = customtkinter.CTkButton(root, text="Speak", command=command)
button_6.pack(pady=10, padx=20)
